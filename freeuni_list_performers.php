...
} elseif ( ( isset($_POST['performer_id']) ) && ( @$_POST['edit'] == "true" ) )
{
	// If we decided to remove a performer
	// Check whether the user is an admin
	if ( $userDBArray['userLevel'] >= 3 )
...
} elseif ( ( isset($_POST['performer_id']) ) && ( @$_POST['unlock'] == "true" ) )
{
	// If we decided to unlock a performer
	// Check whether the user is an admin
	if ( $userDBArray['userLevel'] >= 3 )
	{
		$unlock_performer = $Cmysql->Query("DELETE FROM fu_perf_user_relation WHERE performer_id='" .$Cmysql->EscapeString($_POST['performer_id']). "'");
		
		// $unlock_performer is TRUE or FALSE based on success
		
		if ( $unlock_performer == FALSE )
		{
			// If there were errors unallocating the performer
			$Ctemplate->useTemplate("errormessage", array(
				'THEME_NAME'	=>	$_SESSION['theme_name'], // Theme name
				'PICTURE_NAME'	=>	"Nuvola_filesystems_folder_locked.png", // Locked folder icon
				'TITLE'	=>	"Az előadót nem lehet feloldalni", // Error title
				'BODY'	=>	"A lekérdezést nem sikerült lefuttatni. Az előadó lefoglalva marad.", // Error text
				'ALT'	=>	"SQL hiba" // Alternate picture text
			), FALSE ); // We give an error
		} elseif ( $unlock_performer == TRUE )
		{
			// If there wasn't errors, give success
			$Ctemplate->useTemplate("successbox", array(
				'THEME_NAME'	=>	$_SESSION['theme_name'], // Theme name
				'PICTURE_NAME'	=>	"Nuvola_filesystems_folder_txt.png", // Text folder icon
				'TITLE'	=>	"Az előadó feloldva.", // Success title
				'BODY'	=>	"Az előadó visszakerült szabad státuszba", // Success text
				'ALT'	=>	"Sikeres lekérdezés" // Alternate picture text
			), FALSE ); // We give a success message
			
			$performer_unallocated = $Cmysql->Query("UPDATE fu_performers SET status='unallocated' WHERE id='" .$Cmysql->EscapeString($_POST['performer_id']). "'");
			
			if ( $performer_unallocated == FALSE )
			{
				// If there were errors unallocating the performer
				$Ctemplate->useTemplate("errormessage", array(
					'THEME_NAME'	=>	$_SESSION['theme_name'], // Theme name
					'PICTURE_NAME'	=>	"Nuvola_filesystems_folder_locked.png", // Locked folder icon
					'TITLE'	=>	"Az előadót nem lehet Szabad státuszba tenni", // Error title
					'BODY'	=>	"A lekérdezést nem sikerült lefuttatni.", // Error text
					'ALT'	=>	"SQL hiba" // Alternate picture text
				), FALSE ); // We give an error
			} elseif ( $performer_unallocated == TRUE )
			{
				// If there wasn't errors, give success
				$Ctemplate->useTemplate("successbox", array(
					'THEME_NAME'	=>	$_SESSION['theme_name'], // Theme name
					'PICTURE_NAME'	=>	"Nuvola_filesystems_folder_txt.png", // Text folder icon
					'TITLE'	=>	"Az előadó Szabad státuszba került", // Success title
					'BODY'	=>	"", // Success text
					'ALT'	=>	"Sikeres lekérdezés" // Alternate picture text
				), FALSE ); // We give a success message
			}
...
$Ctemplate->useTemplate("freeuni/list_table_open", array(
	...
	'ADMIN_UNLOCK_HEAD'	=> (($userDBArray['userLevel'] >= 3) && ($_GET['printer'] != 1) ?
		$Ctemplate->useTemplate("freeuni/list_performers_unlock_header", array(
			'THEME_NAME'	=>	$_SESSION['theme_name'], // Theme name
			'ADMINLOGO'	=>	$Ctemplate->useTemplate("adminlogo", array(
				'THEME_NAME'	=>	$_SESSION['theme_name']
			), TRUE) // Administrator logo
		), TRUE) : NULL), // If the user is an admin, give header row for unlock buttons, otherwise, 
...
while ( $row_wc = mysql_fetch_assoc($pWillCome) )
{
	...
	$Ctemplate->useTemplate($row_template, array(
		...
		'ADMIN_UNLOCK_PERFORMER'	=>	( ($userDBArray['userLevel'] >= 3) && ($_GET['printer'] != 1) ? 
			$Ctemplate->useTemplate("freeuni/list_performers_unlock", array(
				'PERFORMER_ID'	=>	$row_wc['id'] // ID of the performer
			), TRUE) : NULL), // If the user is an admin and we aren't in printer friendly mode, give unlock button
