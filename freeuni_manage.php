...
} elseif ( $_SESSION['log_bool'] == TRUE)
{
	...
	} elseif ( ( isset($_POST['emailTemplate']) ) && ( isset($_POST['performer_id']) ) )
	{
		// Give e-mail template if requested
		
		$performer_data = mysql_fetch_assoc($Cmysql->Query("SELECT pName, comments FROM fu_performers WHERE id='" . $Cmysql->EscapeString($_POST['performer_id']). "'"));
		
		$Ctemplate->useTemplate("freeuni/manage_email_template", array(
			'PERFORMER_NAME'	=>	$performer_data['pName'],
			'COMMENTS'	=>	$performer_data['comments'],
			'USER_NAME'	=>	$_SESSION['username']
		), FALSE); // Output template parsed
	} else {
