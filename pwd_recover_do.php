...
} elseif ( ( @$_POST['username'] != NULL ) && ( $_POST['token'] != NULL ) && ( $_POST['step'] == "do" ) )
{
	...
	$setNewPass = $Cmysql->Query("UPDATE users SET pwd='" .md5($Cmysql->EscapeString($_POST['pass1'])). "' WHERE username='" .$Cmysql->EscapeString($_POST['username']). "'"); // Set the new password in database, TRUE if it was successful
