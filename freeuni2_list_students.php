...
if ( $uDBArray['userLevel'] < 2 )
{
	...
	$Ctemplate->useTemplate("errormessage", array(
		...
		'BODY'	=>	"A lap megtekintéséhez Moderátori, vagy magasabb jogkörrel kell rendelkezned", // Error text
		'ALT'	=>	"Házirendhiba" // Alternate picture text
	), FALSE ); // We give an unaviable error
} elseif ( $uDBArray['userLevel'] >= 2 )
{
...
if ( !isset($_POST['osztaly']) )
{
	...
	$osztaly = 0; // Global school class count
	$diak = 0; // Global student count
	...
	while ( $row = mysql_fetch_row($classesResult) )
	{
		...
		$osztaly++; // Add +1 class
		$diak += $row[0]; // Add number of students to global list
	}
	
	$Ctemplate->useStaticTemplate("freeuni2/student_list_form_end", FALSE); // End </form>
	
	echo '<font class="emphasis">Összesen:</font> ' .$osztaly. ' osztály, ' .$diak. ' diák (átlag. ' .($diak/$osztaly). ' diák osztályonként)<br>';
} elseif ( isset($_POST['osztaly']) )
{
	...
	// Make list
	$studentList = $Cmysql->Query("SELECT id, username, userLevel, hour1, hour2, hour3, hour4 FROM users WHERE osztaly='" .$Cmysql->EscapeString($_POST['osztaly']). "' ORDER BY username ASC"); // Query students
	...
	while ( $sRow = mysql_fetch_assoc($studentList) )
	{
		...
		$Ctemplate->useTemplate("freeuni2/student_list_table_row", array(
			...
			'ADMINLOGO'	=>	($sRow['userLevel'] >= 2 ? $Ctemplate->useTemplate("adminlogo", array('THEME_NAME'	=>	"winky"), TRUE) : NULL), // Output administrator icon for moderator or higher level students
